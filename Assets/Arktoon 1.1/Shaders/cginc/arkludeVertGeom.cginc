...
void geom(triangle v2g IN[3], inout TriangleStream<VertexOutput> tristream)
{
    ...
    for (int ii = 0; ii < 3; ii++)
    {
        ...
        #ifdef ARKTOON_OUTLINE_PASS
            float4 _OutlineTexture_var = tex2Dlod (_OutlineTexture, float4( TRANSFORM_TEX(IN[ii].uv0, _OutlineTexture), 0, 0));
            float _OutlineWidthMask_var = tex2Dlod (_OutlineWidthMask, float4( TRANSFORM_TEX(IN[ii].uv0, _OutlineWidthMask), 0, 0));
            float width = _OutlineWidth * _OutlineWidthMask_var;
            o.normalDir = UnityObjectToWorldNormal(IN[ii].normal);
            o.pos = mul( UNITY_MATRIX_VP, mul( unity_ObjectToWorld, IN[ii].vertex ) + float4(normalize(o.normalDir) * (width * 0.01), 0));
            o.uv0 = IN[ii].uv0;
    ...
    //[i].pos =
    //[i].normal = lerp(v[i][0].normal, triNormal, _Width);

            //o.pos = UnityObjectToClipPos(IN[ii].vertex);
            o.pos = UnityObjectToClipPos(IN_b.vertex.xyz + ((IN_1.vertex.xyz + IN_2.vertex.xyz) * 0.5 - IN_b.vertex.xyz) * _OutlineWidth);
            o.uv0 = IN[ii].uv0;
