    ...
    if (_UseOutline == 0 || !i.isOutline) {
        ...
        if (_UseRim) {
            ...
            RimLight = (
                            ...
                            * (0.333 - max(0,min(0.333,abs(dot( normalDirection, viewDirection ))))) * 3
                            //* FresnelLerpFast(float3(0,0,0),float3(1,1,1),abs(dot( normalDirection, viewDirection )))
                            /* pow(
                                min(1.0, 1.0 - max(0, dot(normalDirection * lerp(faceSign, 1, _DoubleSidedFlipBackfaceNormal), viewDirection) ) + _RimUpperSideWidth)
                                , _RimFresnelPower
                                )*/
                            * _RimBlend
                            * _RimColor.rgb
