float4 frag(VertexOutput i) : COLOR {
    ...
    float time = _Time.g;
    #ifdef ARKTOON_SCROLLED
        float2 uv0_scrolled =
            i.uv0
            + float2(fmod(_UScroll * time, 1.0), fmod(time * _VScroll, 1.0));

        float2 uv0_scrolled_emission =
            uv0_scrolled
            + float2(fmod(_UScrollEmission * time, 1.0), fmod(time * _VScrollEmission, 1.0));
    #else
        float2 uv0_scrolled = i.uv0;
        float2 uv0_scrolled_emission = i.uv0;
    #endif
    ...
    float3 _BumpMap_var = UnpackScaleNormal(tex2D(REF_BUMPMAP,TRANSFORM_TEX(uv0_scrolled, REF_BUMPMAP)), REF_BUMPSCALE);
    ...
    float4 _MainTex_var = UNITY_SAMPLE_TEX2D(REF_MAINTEX, TRANSFORM_TEX(uv0_scrolled, REF_MAINTEX));
    ...
    // カスタム陰を使っている場合、directContributionや直前のfinalLightを使い、finalLightを上書きする
        ...
        float3 shadeMixValue = lerp(directLighting, finalLight, _ShadowPlanBDefaultShadowMix);
            ...
            float4 _ShadowPlanBCustomShadowTexture_var = UNITY_SAMPLE_TEX2D_SAMPLER(_ShadowPlanBCustomShadowTexture, REF_MAINTEX, TRANSFORM_TEX(uv0_scrolled, _ShadowPlanBCustomShadowTexture));
            ...
            directContribution2 *= additionalContributionMultiplier;
                ...
                float4 _ShadowPlanB2CustomShadowTexture_var = UNITY_SAMPLE_TEX2D_SAMPLER(_ShadowPlanB2CustomShadowTexture, REF_MAINTEX, TRANSFORM_TEX(uv0_scrolled, _ShadowPlanB2CustomShadowTexture));
    ...
    if (!i.isOutline) {
        ...
        // オプション：Rim
            ...
            float4 _RimTexture_var = tex2D(_RimTexture,TRANSFORM_TEX(uv0_scrolled, _RimTexture));
            RimLight = (
    ...
    float3 _Emission = tex2D(REF_EMISSIONMAP,TRANSFORM_TEX(uv0_scrolled_emission, REF_EMISSIONMAP)).rgb *REF_EMISSIONCOLOR.rgb;
